...
public class DriveBackwardAction extends QQAction{
    double distanceCM;
    ...
    public QQAction run(QQOpMode opMode) {
        ...
        opMode.robot.mecanumDrive.drive(-0.5,0,0);
        double[] distances = opMode.robot.mecanumDrive.getDistanceCM();
        if(-distances[0]>distanceCM){
            opMode.robot.mecanumDrive.drive(0,0,0);
            return nextAction;
        }
        return this;
    }
    public DriveBackwardAction(double distance, DistanceUnit du){
        super();
